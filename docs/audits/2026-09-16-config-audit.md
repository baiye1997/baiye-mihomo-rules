...
## 导入后冷启动故障与热修（2026-09-16）

用户实机导入后节点列表、规则组和图标下载失败。此前的 22 项测试与发布成功不覆盖空缓存远端下载；将 provider 转成 inline / file 的隔离测试掩盖了启动依赖，本次撤回“无剩余阻断问题”对真实首次导入的适用性。

本机证据：两个 Gist 节点 provider 和多个 jsDelivr 规则 provider 同时报 EOF；海外备用 DNS 依赖节点选择，而节点下载又依赖可用解析和出口。独立 Mihomo 使用同一真实配置、物理网卡、空节点与规则缓存（沿用已安装 Geo 数据）复现两个节点缓存及规则缓存均未生成。单独为 Gist 添加 DNS policy 未恢复。图标直连超时，通过临时机场代理返回 200。

修复使用 Mihomo HTTP provider 原生 payload：发布器从两份订阅分别取得节点快照，随配置保存，保留天照／月读前缀；HTTP 更新经节点选择执行。快照获取或解析失败会中止本次发布，避免覆盖线上可启动版本。内核验证保留并解析真实 payload，但不将含凭据的解析输出回显到 CI 日志。上游规则顺序、生产 DNS 和机场凭据均未改写，真实节点不写入代码仓库。

依据：Mihomo v1.19.29 的 adapter/provider/provider.go 在 fetch 初始化前加载备用 payload；https://wiki.metacubex.one/config/proxy-providers/#payload 也说明 HTTP / file 失败时可用 payload 作为备用。

验证：本机用修复版生成配置从空缓存下载两个节点缓存、29 个规则文件，并经独立配置请求 icon 得到 HTTP 200；期间一次订阅更新 EOF 没有导致节点池失效，不能宣称网络请求永不失败。新增保留 HTTP provider 的冷启动回归：两份订阅都返回 503，仍用快照取得全部规则并将 ChatGPT 分到 AI 组；另外检查快照缺失与下载失败阻断发布、不输出订阅 URL。全量 25 项测试通过，无跳过。现用机场配置未被切换或覆写，实机验收应在更新原 Gist 订阅后完成。
